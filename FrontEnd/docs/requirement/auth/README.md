...
| AUTH-02 | Setelah login sukses, access token disimpan dan user diarahkan ke `/admin` |
| AUTH-03 | Request API admin memakai Bearer token otomatis |
| AUTH-04 | User dapat logout; token dihapus dan cache query dibersihkan |
| AUTH-05 | Session refresh via cookie (background / saat 401) — opsional fase 2 |
...
- Cookie refresh hanya dikirim ke path `/api/v1/auth`
